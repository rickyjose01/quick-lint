 ...
 */
// No longer needs direct ESLint dependency since eslint.ts handles execution
 ...
 */
function mapSeverity(eslintSeverity: number, ruleId: string | null): SonarIssue['severity'] {
 ...
 */
function mapCategory(type: SonarIssue['type']): 'bug' | 'vulnerability' | 'code-smell' | 'style' {
  switch (type) {
    case 'bug':
      return 'bug';
    case 'vulnerability':
 ...
 */
export async function runSonarAnalysis(config: QuickLintConfig): Promise<LintResult> {
  ...
  // Use runEslint to get the properly configured parser (TS/React)
  const result = await runEslint(config, { files: ['.'] });

  // Filter to retain ONLY sonarjs issues for the specific SonarQube report
  let totalErrors = 0;
  let totalWarnings = 0;
  const sonarFiles: FileResult[] = [];

  for (const file of result.files) {
    const sonarMsgs = file.messages.filter((m) => m.ruleId?.startsWith('sonarjs/'));
    if (sonarMsgs.length > 0) {
      // Map the ESLint-specific categories to Sonar categories
      const enrichedMsgs = sonarMsgs.map((msg) => ({
        ...msg,
        category: mapCategory(categorizeRule(msg.ruleId)),
      }));

      sonarFiles.push({ filePath: file.filePath, messages: enrichedMsgs });

      for (const m of sonarMsgs) {
        // Ensure severity strictly represents error/warning strings
  ...
  return {
    ...
    fixableCount: 0, // Since SonarJS bugs aren't typically auto-fixable
    files: sonarFiles,
...
export function buildSonarReport(result: LintResult): SonarReport {
  ...
  const files: SonarFileReport[] = result.files.map((file) => {
    const issues: SonarIssue[] = file.messages.map((msg) => {
      ...
      const severity = mapSeverity(msg.severity === 'error' ? 2 : 1, msg.ruleId);
