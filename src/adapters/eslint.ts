...
/**
 * Build the TypeScript plugin configuration or just the parser.
 */
async function buildTypeScriptConfig(config: QuickLintConfig): Promise<Linter.Config[]> {
  if (!config.eslint.typescript) return [];
  const tseslint = await import('typescript-eslint');
  if (config.eslint.enabled) {
    const { recommended } = tseslint.default.configs;
    return Array.isArray(recommended) ? (recommended as Linter.Config[]) : [];
  }
  return [
    {
      files: ['**/*.ts', '**/*.tsx'],
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      languageOptions: { parser: tseslint.default.parser as any },
    },
  ];
}

/**
 * Build the React & React Hooks plugin configuration.
 */
async function buildReactConfig(config: QuickLintConfig): Promise<Linter.Config[]> {
  if (!config.eslint.enabled || !config.eslint.react) return [];
  ...
  configs.push({
    plugins: { 'react-hooks': reactHooksPlugin.default },
    rules: {
  ...
  try {
    ...
    configs.push({
      plugins: { 'jsx-a11y': jsxA11yPlugin.default },
      rules: {
...
export async function buildEslintConfig(config: QuickLintConfig): Promise<Linter.Config[]> {
  ...
  // Base language setup (always needed for React/TS parsing, even if only Sonar is enabled)
  flatConfig.push({
    languageOptions: {
      parserOptions: {
  ...
  flatConfig.push(...(await buildTypeScriptConfig(config)));

  // React & React Hooks plugins
  flatConfig.push(...(await buildReactConfig(config)));
  ...
  if (config.sonarqube.enabled) {
    ...
    // Apply SonarQube rule overrides with correct sonarjs/ prefix
    if (Object.keys(config.sonarqube.rules).length > 0) {
      const mappedSonarRules: Record<string, Linter.RuleEntry> = {};
      for (const [key, val] of Object.entries(config.sonarqube.rules)) {
        const fullKey = key.startsWith('sonarjs/') ? key : `sonarjs/${key}`;
        mappedSonarRules[fullKey] = val as Linter.RuleEntry;
      }
      flatConfig.push({
        rules: mappedSonarRules,
      });
    }
  }
