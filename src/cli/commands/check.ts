...
import type { CheckResult, LintResult, FileResult, QuickLintConfig } from '../../types/index.js';
...
function extractSonarResults(eslintResult: LintResult): LintResult {
  ...
  for (const file of eslintResult.files) {
    const sonarMsgs = file.messages.filter((m) => m.ruleId && m.ruleId.startsWith('sonarjs/'));
...
async function executeEslintCheck(config: QuickLintConfig, result: CheckResult): Promise<void> {
  if (!config.eslint.enabled && !config.sonarqube.enabled) return;
  ...
  try {
    ...
    spinner.succeed(
      `ESLint: ${result.eslint.errorCount} errors, ${result.eslint.warningCount} warnings`,
    );
  } catch (error) {
...
async function executePrettierCheck(config: QuickLintConfig, result: CheckResult): Promise<void> {
  if (!config.prettier.enabled) return;
...
export async function checkCommand(): Promise<void> {
  ...
  await executeEslintCheck(config, result);
  await executePrettierCheck(config, result);
