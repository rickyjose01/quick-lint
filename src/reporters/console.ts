...
/**
 * Format a single lint message into a display string
 */
function formatMessage(msg: {
  ...
  const location = chalk.dim(`${msg.line}:${msg.column}`);
  return `      ${sev}  ${location}  ${msg.message}${rule}`;
}
...
function printLintResult(result: LintResult): void {
  ...
  console.log(
    `  ${icon} ${chalk.bold(toolName)} — ${result.errorCount} errors, ${result.warningCount} warnings (${(result.duration / 1000).toFixed(2)}s)`,
  );
...
function printPrettierResult(result: PrettierResult): void {
  ...
  console.log(
    `  ${icon} ${chalk.bold('Prettier')} — ${result.unformattedFiles.length} unformatted files out of ${result.totalFiles} (${(result.duration / 1000).toFixed(2)}s)`,
  );
